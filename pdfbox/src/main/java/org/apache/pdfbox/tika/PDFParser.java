...
begin_comment
comment|/**  * Tika parser for PDF documents.  *<p>  * This parser can process also encrypted PDF documents if the required  * password is given as a part of the input metadata associated with a  * document. If no password is given, then this parser will try decrypting  * the document using the empty password that's often used with PDFs.  If  * the PDF contains any embedded documents (for example as part of a PDF  * package) then this parser will use the {@link EmbeddedDocumentExtractor}  * to handle them.   * @since Apache PDFBox 1.7.0  */
end_comment
...
name|PASSWORD
init|=
literal|"org.apache.pdfbox.tika.password"
decl_stmt|;
