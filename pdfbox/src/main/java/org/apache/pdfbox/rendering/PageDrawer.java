...
expr_stmt|;
block|}
comment|// returns the image adjusted for applySoftMaskToPaint().
specifier|private
name|BufferedImage
name|adjustImage
parameter_list|(
...
argument_list|)
expr_stmt|;
comment|// PDFBOX-3836: lastClip needs to be reset, because after intersection it is still the same
comment|// object, thus setClip() would believe that it is cached.
name|lastClip
operator|=
literal|null
...
condition|)
block|{
comment|// PDFBOX-4690: bounds doesn't need to be larger than device bounds (OOM risk)
name|Rectangle
name|deviceBounds
init|=
name|deviceConfiguration
...
argument_list|)
decl_stmt|;
comment|// draw the subsampled image
name|drawBufferedImage
argument_list|(
name|pdImage
operator|.
...
block|{
comment|// subsampling not allowed, draw the image
name|drawBufferedImage
argument_list|(
name|pdImage
operator|.
...
parameter_list|)
block|{
comment|// calculate subsampling according to the resulting image size
name|double
name|scale
init|=
name|Math
...
condition|)
block|{
comment|// For very small images it is possible that the subsampling would imply 0 size.
comment|// To avoid problems, the subsampling is set to no less than the smallest dimension.
name|subsampling
operator|=
name|Math
operator|.
...
argument_list|()
decl_stmt|;
comment|// "The upper-left corner of the annotation remains at the same point in
comment|//  default user space; the annotation pivots around that point."
name|graphics
operator|.
name|rotate
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// adjust bbox (x,y) position at the initial scale + cropbox
name|float
name|x
init|=
name|bbox
...
name|image
operator|=
name|create2ByteGrayAlphaImage
argument_list|(
name|width
argument_list|,
name|height
...
literal|null
decl_stmt|;
comment|// Position of this group in parent group's coordinates
name|int
name|backdropX
init|=
literal|0
...
condition|)
block|{
comment|// Use the current page as the parent group.
name|backdropImage
operator|=
name|renderer
operator|.
...
condition|)
block|{
comment|// backdropImage must be included in group image but not in group alpha.
name|g
operator|.
name|drawImage
argument_list|(
...
block|}
block|}
comment|// http://stackoverflow.com/a/21181943/535646
specifier|private
name|BufferedImage
name|create2ByteGrayAlphaImage
parameter_list|(
...
name|length
decl_stmt|;
comment|/**              * Color Model used for raw GRAY + ALPHA              */
specifier|final
name|ColorModel
name|CM_GRAY_ALPHA
init|=
...
argument_list|)
decl_stmt|;
comment|// Init data buffer of type byte
name|DataBuffer
name|buffer
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// Wrap the data buffer in a raster
name|WritableRaster
name|raster
init|=
name|Raster
...
argument_list|)
decl_stmt|;
comment|// Create a custom BufferedImage with the raster and a suitable color model
return|return
operator|new
name|BufferedImage
argument_list|(
...
condition|)
block|{
comment|// The group was already processed. Avoid endless recursion.
return|return
literal|false
return|;
block|}
...
block|}
block|}
comment|// Recursively process nested transparency groups
for|for
control|(
name|COSName
name|name
...
condition|)
block|{
comment|// support seems to be optional, and is approximated by /P and /OCGS
name|LOG
operator|.
name|info
argument_list|(
literal|"/VE entry ignored in Optional Content Membership Dictionary"
argument_list|)
expr_stmt|;
block|}
name|List
