...
name|contentStream
decl_stmt|;
comment|// Hold a reference between the annotations and the page they are on.
comment|// This will only be used in case a PDAnnotationWidget doesn't contain
comment|// a /P entry specifying the page it's on as the /P entry is optional
name|Map
argument_list|<
name|COSDictionary
argument_list|,
...
argument_list|()
decl_stmt|;
comment|// resolve the page from looking at the annotations
if|if
condition|(
name|widget
operator|.
...
name|boolean
name|needsTransformation
init|=
name|isNeedsTransformation
argument_list|(
name|appearanceStream
argument_list|)
decl_stmt|;
...
name|warn
argument_list|(
literal|"Can't retriev annotations for page "
operator|+
name|idx
argument_list|)
expr_stmt|;
...
return|;
block|}
comment|/**      * Check if there is a transformation needed to place the annotations content.      *       * @param appearanceStream      * @return the need for a transformation.      */
specifier|private
name|boolean
name|isNeedsTransformation
parameter_list|(
...
parameter_list|)
block|{
comment|// Check if there is a XObject defined as this is an indication that there should already be a transformation
comment|// in place.
comment|// TODO: A more reliable approach might be to parse the content stream
if|if
condition|(
name|appearanceStream
operator|.
