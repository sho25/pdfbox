...
argument_list|()
expr_stmt|;
comment|// calculate the fontSize
name|float
name|fontSize
init|=
...
argument_list|)
expr_stmt|;
comment|// the font has already been set so only write the remaining parts of the DA string
name|daWriter
operator|.
name|writeTokens
argument_list|(
...
expr_stmt|;
block|}
comment|// if the font size is 0 the size depends on the content
if|if
condition|(
name|fontSize
operator|==
...
expr_stmt|;
block|}
comment|// restore to default size for multiline text
if|if
condition|(
name|fontSize
operator|==
