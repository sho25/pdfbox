...
expr_stmt|;
block|}
comment|/**      * Some Type 1 fonts have an invalid Length1, which causes the binary segment of the font      * to be truncated, see PDFBOX-2350, PDFBOX-3677.      *      * @param bytes Type 1 stream bytes      * @param length1 Length1 from the Type 1 stream      * @return repaired Length1 value      */
specifier|private
name|int
name|repairLength1
parameter_list|(
...
name|offset
operator|=
name|findBinaryOffsetAfterExec
argument_list|(
name|bytes
argument_list|,
name|offset
...
condition|)
block|{
comment|// 2nd try with brute force
name|offset
operator|=
name|findBinaryOffsetAfterExec
argument_list|(
...
condition|)
block|{
comment|// genericFont not needed, thus simplified code
comment|// this is important on systems with no installed fonts
if|if
condition|(
operator|!
name|encoding
