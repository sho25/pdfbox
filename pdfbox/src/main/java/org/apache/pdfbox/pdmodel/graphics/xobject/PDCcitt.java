...
literal|0
decl_stmt|;
comment|// determine the index for the CCITT-filter
name|COSBase
name|filters
init|=
name|stream
...
argument_list|()
decl_stmt|;
comment|// most likely there is no colorspace as a CCITT-filter uses 1-bit values mapped to black/white
comment|// in some rare cases other colorspaces maybe used such as an indexed colorspace, see PDFBOX-1638
if|if
condition|(
name|colorspace
operator|instanceof
...
expr_stmt|;
block|}
comment|/*          * If we have an image mask we need to add an alpha channel to the data          */
if|if
condition|(
name|hasMask
argument_list|()
...
name|IOException
block|{
comment|// We should use another format than TIFF to get rid of the TiffWrapper
name|InputStream
name|data
init|=
operator|new
