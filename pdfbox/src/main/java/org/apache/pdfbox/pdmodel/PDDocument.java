...
expr_stmt|;
block|}
comment|/**      * This will add a signature to the document.       *      * @param sigObject is the PDSignature model      * @param signatureInterface is a interface which provides signing capabilities      * @param options       * @throws IOException if there is an error creating required fields      */
specifier|public
name|void
name|addSignature
parameter_list|(
...
literal|false
return|;
comment|/*boolean retval = false;         if( password == null )         {             password = "";         }         PDFEncryption encryptor = new PDFEncryption();         PDEncryptionDictionary encryptionDictionary = getEncryptionDictionary();         if( encryptionDictionary == null )         {             throw new IOException( "Error: Document is not encrypted" );         }         else         {             if( encryptionDictionary instanceof PDStandardEncryption )             {                 COSString documentID = (COSString)document.getDocumentID().get( 0 );                 PDStandardEncryption standard = (PDStandardEncryption)encryptionDictionary;                 retval = encryptor.isOwnerPassword(                     password.getBytes("ISO-8859-1"),                     standard.getUserKey(),                     standard.getOwnerKey(),                     standard.getPermissions(),                     documentID.getBytes(),                     standard.getRevision(),                     standard.getLength()/8 );             }             else             {                 throw new IOException( "Error: Encyption dictionary is not 'Standard'" +                     encryptionDictionary.getClass().getName() );             }         }         return retval;*/
block|}
...
return|;
block|}
comment|/**      * Sets security handler if none is set already.      *       * @param _sHandler  security handler to be assigned to document      * @return<code>true</code> if security handler was set,<code>false</code>      *          otherwise (a security handler was already set)      */
specifier|public
name|boolean
