...
comment|// How it could be done by reusing some of the code from the line handler
comment|// 1) do a transform so that first and last "arms" are imagined flat
comment|// (like in line handler)
comment|// 2) refactor + reuse the line handler code that draws the ending shapes
