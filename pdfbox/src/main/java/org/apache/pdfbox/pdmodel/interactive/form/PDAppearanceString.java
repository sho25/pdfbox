...
argument_list|()
decl_stmt|;
comment|/**      * Constructs a COSAppearance from the given field.      *      * @param theAcroForm the AcroForm that this field is part of.      * @param field the field which you wish to control the appearance of      */
specifier|public
name|PDAppearanceString
parameter_list|(
name|PDAcroForm
...
argument_list|)
expr_stmt|;
comment|// replace the first string only if the appearance stream
comment|// consists of more than one text
break|break;
block|}
block|}
name|int
...
argument_list|)
decl_stmt|;
comment|// the existing appearance stream may contain more than one text
comment|// so that we shall replace the first with the new value
comment|// and skip the remaining parts
if|if
condition|(
name|indexOfString
operator|+
...
condition|)
block|{
comment|// reuse the fontsize of an existing apperance stream
name|int
name|fontIndex
init|=
name|tokens
