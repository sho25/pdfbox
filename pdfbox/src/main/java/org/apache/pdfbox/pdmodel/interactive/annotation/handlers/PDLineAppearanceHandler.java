...
argument_list|)
decl_stmt|;
comment|// current strategy for arrows: angle 30°, arrow arm length = 9 * line width
comment|// cos(angle) = x position
comment|// sin(angle) = y position
comment|// this comes very close to what Adobe is doing
comment|//TODO all four arrow paintings can be put into one method
comment|//     params: x, y, orientation, width, hasStroke, hasBackground
if|if
condition|(
name|PDAnnotationLine
operator|.
...
block|}
block|}
comment|/**      * Add a square diamond shape to the path.      *      * @param cs Content stream      * @param x      * @param y      * @param r Radius (to a corner)      *       * @throws IOException If the content stream could not be written      */
specifier|private
name|void
name|drawDiamond
parameter_list|(
