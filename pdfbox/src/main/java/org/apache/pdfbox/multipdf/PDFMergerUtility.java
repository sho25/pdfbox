...
argument_list|)
expr_stmt|;
comment|// PDFBOX-4429: remove bogus StructParent(s)
for|for
control|(
name|PDPage
name|page
...
argument_list|)
expr_stmt|;
comment|// check the booleans - set to true if one is set and true
if|if
condition|(
name|srcViewerPreferences
operator|.
...
name|IOException
block|{
comment|// make new /K with array that has the input /K entries
name|COSArray
name|newKArray
init|=
operator|new
...
argument_list|()
operator|+
literal|" already exists in destination IDTree"
argument_list|)
expr_stmt|;
block|}
else|else
...
return|;
block|}
comment|// PDNumberTreeNode.getNumbers() only brings one level, this is why we need this
comment|// might be made public at a later time, or integrated into PDNumberTreeNode.
specifier|static
name|Map
argument_list|<
name|Integer
...
condition|)
block|{
comment|// already exists, but identical
continue|continue;
block|}
if|if
condition|(
...
argument_list|()
operator|+
literal|" already exists in destination RoleMap"
argument_list|)
expr_stmt|;
block|}
else|else
