...
block|}
block|}
comment|// are there any replaced object keys
if|if
condition|(
operator|!
name|replacedKeys
...
control|)
block|{
comment|// remove to be replaced entries first
name|offsets
operator|.
name|put
argument_list|(
...
name|debug
argument_list|(
literal|"Object key replaced within xref table: "
operator|+
name|oldObjectKey
operator|+
literal|" -> "
...
block|}
block|}
comment|/**      * Check if the given object can be found at the given offset.      *       * Returns the given object key if the object can be found.      * Returns a new object key if an object can be found but the number doesn't fit.      * Returns null if non valid object can be found at the given offset.      *       * @param objectKey the object we are looking for      * @param offset the offset where to look      * @return returns the object key of the object at the given offset      * @throws IOException if something went wrong      */
specifier|private
name|COSObjectKey
name|checkObjectKeys
parameter_list|(
...
try|try
block|{
comment|// can't find the object key, maybe we are looking for the wrong number
name|pdfSource
operator|.
name|seek
argument_list|(
...
condition|)
block|{
comment|// found a valid object number, return a new COSObjectKey
return|return
operator|new
name|COSObjectKey
argument_list|(
