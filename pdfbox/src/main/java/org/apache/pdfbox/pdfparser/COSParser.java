...
name|debug
argument_list|(
literal|"Performing brute force search for last startxref entry"
argument_list|)
expr_stmt|;
name|long
name|bfOffset
init|=
name|bfSearchForLastStartxrefEntry
argument_list|()
decl_stmt|;
name|boolean
name|offsetIsValid
...
argument_list|)
expr_stmt|;
comment|// use the new offset only if it is a valid pointer to a xref table
return|return
name|offsetIsValid
condition|?
name|bfOffset
...
condition|)
block|{
comment|// remove all parsed objects which are part of an object stream
name|Set
argument_list|<
name|Long
argument_list|>
...
condition|)
block|{
comment|// if the pdf wasn't cut off in the middle the last object id has to added here
comment|// so that it can't get lost as there isn't any subsequent object id
name|bfSearchCOSObjectKeyOffsets
operator|.
name|put
argument_list|(
...
block|}
block|}
comment|/**      * Brute force search for the last startxref entry.      *       * @throws IOException if something went wrong      */
specifier|private
name|long
name|bfSearchForLastStartxrefEntry
parameter_list|()
