...
expr_stmt|;
block|}
comment|/**      * Constructor to allow control over RandomAccessFile.      * Also enables parser to skip corrupt objects to try and force parsing      * @param input The input stream that contains the PDF document.      * @param force When true, the parser will skip corrupt pdf objects and      * will continue parsing at the next object in the file      * @param useScratchFiles enables the usage of a scratch file if set to true      *      * @throws IOException If there is an error initializing the stream.      */
specifier|public
name|PDFParser
parameter_list|(
name|InputStream
...
parameter_list|)
block|{
comment|// if the PDF is corrupt then close the document and clear all resources to it
if|if
condition|(
name|document
operator|!=
...
block|}
block|}
comment|/**      * Check whether streams with previously unknown length have the correct      * length and fix that length if needed.      *      * @throws IOException      */
specifier|private
name|void
name|fixStreamsLength
parameter_list|()
...
condition|)
block|{
comment|// adjust the length, but only if the difference is> 2,
comment|// i.e. don't bother with CR LF differences
name|LOG
operator|.
name|warn
argument_list|(
...
name|pb
decl_stmt|;
comment|// remember streams without length to check them later
name|COSBase
name|streamLength
init|=
name|strmObj
