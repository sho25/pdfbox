...
name|FileInputStream
argument_list|(
literal|"target/pdfs/PDFBOX-4308.pdf"
argument_list|)
init|)
block|{
name|inputFileAsByteArray
...
control|)
block|{
comment|// strings must be in different dictionaries so that the actual
comment|// encryption key changes
name|COSDictionary
name|dict
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// need two different keys so that there are both encrypted and decrypted COSStrings
comment|// with value "0"
name|dict
operator|.
name|setString
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// Older encryption allows to get the user password when the owner password is known
name|PDEncryption
name|encryption
init|=
name|encryptedDoc
