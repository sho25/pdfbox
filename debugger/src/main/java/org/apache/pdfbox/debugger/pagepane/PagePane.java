...
decl_stmt|;
comment|// more ideas:
comment|// https://stackoverflow.com/questions/16440159/dragging-of-shapes-on-jpanel
specifier|public
name|PagePane
parameter_list|(
name|PDDocument
...
control|)
block|{
comment|// check if the annotation widget is on this page
comment|// (checking widget.getPage() also works, but it is sometimes null)
if|if
condition|(
name|dictionarySet
operator|.
...
name|IOException
block|{
comment|// rendering can take a long time, so remember all options that are used later
name|float
name|scale
init|=
name|ZoomMenu
