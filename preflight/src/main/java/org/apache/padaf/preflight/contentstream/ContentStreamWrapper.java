...
name|IOException
block|{
comment|// ---- Here is a copy of the super method because the else block is
comment|// different. (If the operator is unknown, throw an exception)
name|String
name|operation
init|=
name|operator
...
expr_stmt|;
block|}
comment|// --- Process Specific Validation
comment|// --- The Generic Processing is useless for PDFA validation
if|if
condition|(
literal|"BI"
operator|.
...
name|IOException
block|{
comment|// ---- For a Text operator, the arguments list should contain only one
comment|// COSString object
if|if
condition|(
literal|"\""
operator|.
...
expr_stmt|;
block|}
comment|// FontContainer fontContainer = documentHandler.retrieveFontContainer(font);
name|AbstractFontContainer
name|fontContainer
init|=
name|documentHandler
...
expr_stmt|;
block|}
comment|// ---- According to the length of the character encoding,
comment|// convert the character to CID
name|int
name|cid
init|=
literal|0
