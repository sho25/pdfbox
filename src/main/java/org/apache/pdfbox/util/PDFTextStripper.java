...
argument_list|)
decl_stmt|;
comment|/* In the wild, some PDF encoded documents put diacritics (accents on               * top of characters) into a separate Tj element.  When displaying them               * graphically, the two chunks get overlayed.  With text output though,               * we need to do the overlay. This code recombines the diacritic with               * its associated character.              */
comment|// First, do we even care. The assumption is we do IFF
comment|// we have a single diacritic.
name|boolean
name|wasMerged
init|=
literal|false
...
block|}
block|}
comment|// if we could not merge with the previous entry, add it to the list
if|if
condition|(
name|wasMerged
operator|==
